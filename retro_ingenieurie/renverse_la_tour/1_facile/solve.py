def tour1(password):
    string = str("".join( "".join(password[::-1])[::-1])[::-1])
    return [ord(c) for c in string]


def tour2(password):
    new = []
    i = 0
    while password != []:
        new.append(password[password.index(password[i])])
        new.append(password[password.index(password[i])] + password[password.index(password[ i + 1 %len(password)])])
        password.pop(password.index(password[i]))
        i += int('qkdj', base=27) - int('QKDJ', base=31) + 267500
    return new

def tour3(password):
    mdp = ['l', 'x', 'i', 'b', 'i', 'i', 'q', 'u', 'd', 'v', 'a', 'v', 'b', 'n', 'l', 'v', 'v', 'l', 'g', 'z', 'q', 'g', 'i', 'u', 'd', 'u', 'd', 'j', 'o', 'r', 'y', 'r', 'u', 'a']
    for i in range(len(password)):
        mdp[i], mdp[len(password) - i -1 ] = chr(password[len(password) - i -1 ] + i % 4),  chr(password[i] + i % 4)

    return "".join(mdp)

mdp = "¡P6¨sÉU1T0d¸VÊvçu©6RÈx¨4xFw5"
password = []

for i in range(len(mdp)):
    password.append(ord(mdp[i]))

password2 = ['l', 'x', 'i', 'b', 'i', 'i', 'q', 'u', 'd', 'v', 'a', 'v', 'b', 'n', 'l', 'v', 'v', 'l', 'g', 'z', 'q', 'g', 'i', 'u', 'd', 'u', 'd', 'j', 'o', 'r', 'y', 'r', 'u', 'a']

for i in range(len(password)):
    password2[i]                     = password[len(password) - i - 1] - i % 4
    password2[len(password) - i - 1] = password[i] - i % 4

password3 = []
for i in range(0, len(password2), 2):
    password3.append(chr(password2[i]))

password3.reverse()

print("".join(password3))
print()

if tour3(tour2(tour1("".join(password3)))) == "¡P6¨sÉU1T0d¸VÊvçu©6RÈx¨4xFw5":
    print("Bravo ! Le flag est 404CTF{" + "".join(password3) + "}")
else :
    print("Désolé, le mot-de-passe n'est pas correct")
